...
def cadastrar_usuario(nome):
    ...
    if not arq_existe(arq, criar=False):
        if cria_arq(arq):
            escreva_arq(arq,[],ext='json',sobrescreva= True)
        else:
            print('ERRO ao criar o arquivo!')
            return False
    users = ler_arq(arq,ext = 'json')
    ...
    for cont, u in enumerate(sorted(dados, key= lambda i: i['id'])):
        ...
        usuario.definir_id(len(users)+1)
...
def diferenciar_apelido(apelido):
    """
    -> Analisa se um apelido é repetido.

    Verifica no banco de dados da classe Usuario se há usuários com o mesmo apelido.
    Caso positivo, os diferencia pelo nome e ID.
    :param apelido: Apelido a ser analisado
    :return:
    """
    pass
